...
struct process *process_spawn(const char *name, const void *elf, size_t size, bool detached,
    ...
    bool ok = process->address_space &&
              elf_load(process->address_space, elf, size, &process->entry,
                       &process->personality, &reason) == 0;
...
void process_exit(int code) {
    process_current()->exit_code = code;
    thread_exit();
}
